...
const BANNER_URL = 'https://i.imgur.com/STS2CwI.png'; // Zeus Clan welcome banner
...
if (!role) return message.reply(`❌ Role \`${roleName}\` not found.`); await target.roles.add(role); return message.reply({ embeds: [zeusEmbed('✅ Role Assigned', `Gave **${role.name}** to **${target.user.tag}**`)] });
}
});
